...
//        TreeNode.printLevelWise(tree);
    ...
    public static List<List<Integer>> levelOrder(TreeNode root) {
        ...
        BigInteger miss = BigInteger.ZERO;
        BigInteger levelCount = BigInteger.ONE;
        while (!q.isEmpty()) {
            BigInteger eleCount = miss.add(BigInteger.valueOf(q.size()));
//            System.out.println("Size = " + q.size() + " Ele:" + eleCount + " - " + levelCount);
            if (eleCount.equals(levelCount)) {
                result.add(subList);
//                System.out.println(miss + "->" + eleCount + " " + levelCount + " Adding " + subList);
                subList = new ArrayList<>();
                miss = miss.shiftLeft(1);
                levelCount = levelCount.shiftLeft(1);
            }
            TreeNode treeNode = q.poll();
            subList.add(treeNode.val);
            if (treeNode.left != null) q.offer(treeNode.left);
            else miss = miss.add(BigInteger.ONE);
    ...
    public static List<List<Integer>> levelOrder3(TreeNode root) {
        ...
        while (!q.isEmpty()) {
            ...
            if (pow2(q.size() + miss)) {
                result.add(subList);
//                System.out.println(miss + "->" + (miss + q.size()) + " Adding " + subList);
                subList = new ArrayList<>();
                miss *= 2;
            }
        }
